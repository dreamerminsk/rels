...
namespace fstats
{
    public partial class Form1 : Form
    {
        ...
        {
            var results = await Task.WhenAll(GetAustraliaWomenSingles(),
                GetFranceWomenSingles(), GetUKWomenSingles(), GetUSWomenSingles());
        ...
        private async Task<List<string>> GetUKWomenSingles()
        {
        ...
        private async Task<List<string>> GetUSWomenSingles()
        {
            var page = await htmlWeb.LoadFromWebAsync("https://en.wikipedia.org/wiki/List_of_US_Open_women%27s_singles_champions");
            var refs = page.DocumentNode.SelectNodes("//div[@class='navbox'][2]/table/tbody/tr[3]/td[@class='navbox-list navbox-odd']/div/ul/li/a");
            return refs.Select(r => HtmlEntity.DeEntitize(r?.Attributes["href"]?.Value)).ToList();
        }
